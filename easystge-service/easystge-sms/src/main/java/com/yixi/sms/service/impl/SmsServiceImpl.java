...
public class SmsServiceImpl implements SmsService{
     ...
     */
    @Override
    public boolean sendMailForgotVc(String code, String email) throws BusinessException {
        //判断邮箱号码是否为空
        if (!StringUtils.hasLength(email)){
            throw new BusinessException(EventCode.NULL_ERROR,"邮箱号码为空");
        }
        String vcKey = EmailConstant.EMAIL_VC_FORGOT_KEY +email;    // redis保存验证码的 key
        ...
        code = code+"_"+System.currentTimeMillis();     //给验证码拼接上当前系统时间
        try {
            MailUtil.send(email,"邮箱找回密码",content,true);
        }catch (Exception e){
            e.printStackTrace();
            throw new BusinessException(EventCode.SYSTEM_ERROR, "验证码发送失败");
        }
